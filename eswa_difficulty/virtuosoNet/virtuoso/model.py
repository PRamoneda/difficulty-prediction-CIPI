...
from eswa_difficulty.virtuosoNet.virtuoso import note_embedder
...
class VirtuosoNet(nn.Module):
    def __init__(self, net_param, data_stats, args):
        ...
        self.args = args
        self.save_embedding_path = args.save_embedding
    ...
    def forward(self, x, y, edges, note_locations, initial_z=None):
        x_embedded = self.note_embedder(x)
        score_embedding = self.score_encoder(x_embedded, edges, note_locations)
        score_embedding["x"] = x.cpu()
        # save_json()
        torch.save(score_embedding, self.save_embedding_path)
        exit()
